...
struct Node {
/* Sometimes a key is also stored and used in the functions */ int data;
    struct Node *next;
...
int main(void) {
/* Sometimes in a doubly linked list the last node is also stored */ struct Node *head = NULL;
...
void print_list(struct Node *headNode) {
/* Iterate through the list and print out the data member of each node */ struct Node *i;
...
*/
    ...
    if (*pheadNode == NULL) { /* The list is empty */ *pheadNode = currentNode;
        return;
    }
...
void insert_at_end(struct Node **pheadNode, int value) {
    ...
    */
    ...
    while (i->next != NULL) { /* Go to the end of the list */ i = i->next;
    }
    i->next = currentNode;
